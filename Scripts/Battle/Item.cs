...
public abstract class Item{
	...
	private StatModifier[] modifiers {get;}


	public virtual StatModifier[] applyModifiersOnAttack(Entity entity, int damage) {//this is used for weapons and items that affect attack damage mostly
		return modifiers[];	
	}
	public virtual StatModifier[] applyModifiersOnDamage(EntityPlayer player, int damage) {//used when the player takes damage from an attack, use only for special effects e.g. effects that trigger based on the amount of damage taken
		return null;
	}
	public virtual StatModifier[] applyModifiersPassive(EntityPlayer player) {//used for stat modifiers that are just always on, use this for normal calculuation of armor defense modifiers and stuff
		return modifiers[];
	}
}
